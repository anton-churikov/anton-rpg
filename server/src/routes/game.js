...
const SKILL_XP_THR = [0,100,250,500,850,1300,1900,2700,3700,5000];
...
skillsRouter.patch('/:id', requireAuth, (req, res) => {
  ...
  const now = new Date().toISOString();
  const prevLevel = skillLevel(s.xp);
  const newXP = xp ?? s.xp;
  const newLevel = skillLevel(newXP);
  db.prepare('UPDATE skills SET name=?,category=?,xp=?,notes=?,resources=?,updatedAt=? WHERE id=? AND userId=?')
    .run(name??s.name, category??s.category, newXP, notes??s.notes, JSON.stringify(resources??JSON.parse(s.resources||'[]')), now, req.params.id, req.userId);
  const updated = db.prepare('SELECT * FROM skills WHERE id = ?').get(req.params.id);
  res.json({ ...updated, resources: JSON.parse(updated.resources), leveledUp: newLevel > prevLevel, newLevel });
});
...
skillsRouter.post('/:id/xp', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE skills SET xp = xp + ?, updatedAt = ? WHERE id = ? AND userId = ?')
    .run(amount, now, req.params.id, req.userId);
  const updated = db.prepare('SELECT * FROM skills WHERE id = ?').get(req.params.id);
  res.json({ ...updated, resources: JSON.parse(updated.resources), leveledUp: newLevel > prevLevel, newLevel });
});
...
skillsRouter.delete('/:id', requireAuth, (req, res) => {
  const db = getDb();
  const s = db.prepare('SELECT id FROM skills WHERE id = ? AND userId = ?').get(req.params.id, req.userId);
  if (!s) return res.status(404).json({ error: 'Skill not found' });
...
questsRouter.get('/', requireAuth, (req, res) => {
  const db = getDb();
  const quests = db.prepare(questJoin + ` ORDER BY CASE q.type WHEN 'boss' THEN 0 WHEN 'main' THEN 1 WHEN 'side' THEN 2 ELSE 3 END, q.createdAt DESC`).all(req.userId);
  res.json(quests);
});
...
questsRouter.post('/', requireAuth, (req, res) => {
  ...
  const now = new Date().toISOString();
  const id = uuid();
  db.prepare('INSERT INTO quests (id,userId,title,description,type,status,relatedSkillId,xpReward,deadline,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
    .run(id, req.userId, title.trim(), description||'', type||'main', 'active', relatedSkillId||null, xpReward||(xpMap[type||'main']||200), deadline||null, now, now);
...
questsRouter.patch('/:id', requireAuth, (req, res) => {
  ...
  if (completing) {
    ...
    db.prepare('INSERT INTO activity_log (id,userId,type,description,xpEarned,relatedId,createdAt) VALUES (?,?,?,?,?,?,?)')
      .run(uuid(), req.userId, 'quest_complete', `Quest completed: ${ex.title}`, reward, req.params.id, now);
  }

  db.prepare('UPDATE quests SET title=?,description=?,type=?,status=?,relatedSkillId=?,xpReward=?,deadline=?,completedAt=?,updatedAt=? WHERE id=? AND userId=?')
    .run(title??ex.title, description??ex.description, type??ex.type, newStatus,
      relatedSkillId!==undefined?relatedSkillId:ex.relatedSkillId, xpReward??ex.xpReward,
      deadline!==undefined?deadline:ex.deadline, completedAt, now, req.params.id, req.userId);
...
tasksRouter.post('/', requireAuth, (req, res) => {
  ...
  db.prepare('INSERT INTO tasks (id,userId,title,description,difficulty,status,relatedSkillId,xpReward,dueDate,createdAt,updatedAt) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
    .run(id, req.userId, title.trim(), description||'', diff, status||'todo', relatedSkillId||null, diffXP[diff]||50, dueDate||null, now, now);
...
tasksRouter.patch('/:id', requireAuth, (req, res) => {
  ...
  if (completing) {
    ...
    db.prepare('INSERT INTO activity_log (id,userId,type,description,xpEarned,relatedId,createdAt) VALUES (?,?,?,?,?,?,?)')
      .run(uuid(), req.userId, 'task_complete', `Task completed: ${ex.title}`, reward, req.params.id, now);
    // Update streak
  ...
  db.prepare('UPDATE tasks SET title=?,description=?,difficulty=?,status=?,relatedSkillId=?,xpReward=?,dueDate=?,completedAt=?,updatedAt=? WHERE id=? AND userId=?')
    .run(title??ex.title, description??ex.description, newDiff, newStatus,
      relatedSkillId!==undefined?relatedSkillId:ex.relatedSkillId, diffXP[newDiff]||ex.xpReward,
      dueDate!==undefined?dueDate:ex.dueDate, completedAt, now, req.params.id, req.userId);
...
activityRouter.get('/', requireAuth, (req, res) => {
  const db = getDb();
  const log = db.prepare('SELECT * FROM activity_log WHERE userId = ? ORDER BY createdAt DESC LIMIT 50').all(req.userId);
  res.json(log);
});
...
eventsRouter.get('/', requireAuth, (req, res) => {
  ...
  let q = `
    SELECT e.*, s.name as skillName, s.category as skillCategory,
           qt.title as questTitle
    FROM events e
